...
public class TestResultToALMUploader extends Recorder implements Serializable, MatrixAggregatable, SimpleBuildStep {
	...
	private Map<String, String> testCaseFields;
	private Map<String, String> testSetFields;
	...
	public Map<String, String> getTestCaseFields() {
		return testCaseFields;
	}

	public void setTestCaseFields(Map<String, String> testCaseFields) {
		this.testCaseFields = testCaseFields;
	}

	public Map<String, String> getTestSetFields() {
		return testSetFields;
	}

	public void setTestSetFields(Map<String, String> testSetFields) {
		this.testSetFields = testSetFields;
	}

	@DataBoundConstructor
	public TestResultToALMUploader(String almServerName, String credentialsId, String almDomain, String clientType,
			String almProject, String testingFramework, String testingTool, String almTestFolder,
			String almTestSetFolder, String almTimeout, String testingResultFile, String jenkinsServerUrl,
			Map<String, String> testCaseFields, Map<String, String> testSetFields) {
	...
	private UsernamePasswordCredentials getCredentialsById(String credentialsId, Run<?, ?> run,
			ExternalEntityUploadLogger logger) {
		UsernamePasswordCredentials credentials = CredentialsProvider.findCredentialById(credentialsId,
				StandardUsernamePasswordCredentials.class, run, URIRequirementBuilder.create().build());
	...
	private String getAlmServerUrl(String almServerName) {
		AlmServerSettingsModel[] almServers = Hudson.getInstance()
				.getDescriptorByType(AlmServerSettingsBuilder.DescriptorImpl.class).getInstallations();
	...
	/**
	 * This works for jelly <f:option selected=
	 * "${almServer.almServerName==instance.almServerSettingsModel.almServerName}"
	...
	public void perform(@Nonnull Run<?, ?> run, @Nonnull FilePath filePath, @Nonnull Launcher launcher,
		...
		logger.log("INFO: 'Upload test result to ALM' Post Build Step is being invoked with testSet-fields."
				+ testSetFields);
		logger.log("INFO: 'Upload test result to ALM' Post Build Step is being invoked with testCase-fields."
				+ testCaseFields);
		...
		uploadTestResultToAlmModel = new UploadTestResultToAlmModel(almServerName, credentials.getUsername(),
				credentials.getPassword().getPlainText(), almDomain, clientType, almProject, testingFramework,
				testingTool, almTestFolder, almTestSetFolder, almTimeout, testingResultFile, jenkinsServerUrl,
		...
		if (ds.getIncludedFilesCount() == 0) {
			...
			for (String fileName : files) {
				...
				AlmRestInfo loginInfo = new AlmRestInfo(serverUrl,
						Util.replaceMacro(uploadTestResultToAlmModel.getAlmDomain(), varResolver), clientType,
						Util.replaceMacro(uploadTestResultToAlmModel.getAlmProject(), varResolver),
						uploadTestResultToAlmModel.getAlmUserName(), uploadTestResultToAlmModel.getAlmPassword(),
				...
				try {
					service.UploadExternalTestSet(loginInfo, fullpath,
							Util.replaceMacro(uploadTestResultToAlmModel.getAlmTestSetFolder(), varResolver),
							Util.replaceMacro(uploadTestResultToAlmModel.getAlmTestFolder(), varResolver),
							uploadTestResultToAlmModel.getTestingFramework(),
							uploadTestResultToAlmModel.getTestingTool(), String.valueOf(run.getNumber()),
							run.getParent().getDisplayName(), runUrl, uploadTestResultToAlmModel.getTestSetFields(),
							uploadTestResultToAlmModel.getTestCaseFields());
	...
	public static class DescriptorImpl extends BuildStepDescriptor<Publisher> {
			...
			return new StandardUsernameListBoxModel().includeEmptyValue()
					.includeAs(
							project instanceof Queue.Task ? Tasks.getAuthenticationOf((Queue.Task) project)
									: ACL.SYSTEM,
							project, StandardUsernamePasswordCredentials.class, URIRequirementBuilder.create().build())
		...
		public FormValidation doCheckCredentialsId(@AncestorInPath Item project, @QueryParameter String value) {
			for (ListBoxModel.Option o : CredentialsProvider.listCredentials(StandardUsernamePasswordCredentials.class,
					...
					URIRequirementBuilder.create().build(), new IdMatcher(value))) {
