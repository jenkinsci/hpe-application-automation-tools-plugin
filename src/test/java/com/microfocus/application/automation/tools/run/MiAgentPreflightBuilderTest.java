...
public class MiAgentPreflightBuilderTest {
    ...
    public void checkMiAgentExecutable_throwsWhenParentIsNull() throws Exception {
        ...
        assertTrue(ex.getMessage().contains("shared workspace root"));
    ...
    public void checkMiAgentExecutable_throwsWhenExeNotFound() throws Exception {
        ...
        AbortException ex = assertThrows(AbortException.class,
                () -> builder.perform(build, workspace, launcher, listener));
        assertTrue(ex.getMessage().contains("C:\\Jenkins\\workspace\\mi-agent.exe"));
        verify(launcher, never()).launch();
    }
    ...
    public void checkMiAgentExecutable_throwsWhenExeIsDirectory() throws Exception {
        ...
        AbortException ex = assertThrows(AbortException.class,
                () -> builder.perform(build, workspace, launcher, listener));
        assertTrue(ex.getMessage().contains("mi-agent.exe not found"));
        verify(launcher, never()).launch();
    }
    ...
    public void checkMiAgentExecutable_proceedsToSignerValidationWhenExePresent() throws Exception {
        ...
        AbortException ex = assertThrows(AbortException.class,
                () -> builder.perform(build, workspace, launcher, listener));
        assertTrue(ex.getMessage().contains("Windows agent"));
        verify(launcher, never()).launch();
    }
    ...
    public void validateSignerName_throwsWhenPowerShellExitCodeNonZero() throws Exception {
        ...
        AbortException ex = assertThrows(AbortException.class,
                () -> builder.perform(build, workspace, launcher, listener));
        assertTrue(ex.getMessage().contains("PowerShell exit code=1"));
    ...
    public void validateSignerName_throwsWhenExeNotSigned() throws Exception {
        ...
        AbortException ex = assertThrows(AbortException.class,
                () -> builder.perform(build, workspace, launcher, listener));
        assertTrue(ex.getMessage().contains("not digitally signed"));
    }
    ...
    public void validateSignerName_throwsWhenSignerNameDoesNotMatch() throws Exception {
        ...
        AbortException ex = assertThrows(AbortException.class,
                () -> builder.perform(build, workspace, launcher, listener));
        assertTrue(ex.getMessage().contains("Some Other Signer"));
        assertTrue(ex.getMessage().contains(MiAgentPreflightBuilder.EXPECTED_SIGNER_NAME));
    }
    ...
    public void perform_succeedsWithValidSigner() throws Exception {
        ...
        PrintStream logStream = new PrintStream(logCapture, true, StandardCharsets.UTF_8);
        when(listener.getLogger()).thenReturn(logStream);

        builder.perform(build, workspace, launcher, listener);

        String logs = logCapture.toString(StandardCharsets.UTF_8);
        assertTrue(logs.contains("[MI Agent][PREFLIGHT] mi-agent.exe found at: C:\\Jenkins\\workspace\\mi-agent.exe"));
        assertTrue(logs.contains("[MI Agent][PREFLIGHT] mi-agent.exe signer validated"));
    }
    ...
    @Test
    public void extractOutputValue_returnsFirstMatchingLineAcrossPlatforms() {
        String output = "STATUS=Valid\r\nSIGNER=Primary\r\nSIGNER=Secondary\n";

        String signer = MiAgentPreflightBuilder.extractOutputValue(output, "SIGNER=");

        assertEquals("Primary", signer);
    }
